...
export class Base_Field extends HTMLElement {
    ...
    connectedCallback() {
        ...
        // 1. Rastreamento de Registro de Evento Global
        console.log(`%c[EVENT-REG] %c${this.id}: Ouvinte 'languageChanged' registrado no Window.`, "color: #fd7e14; font-weight: bold;", "color: #333;");

        // 1. // Escuta o evento global de troca de idioma e associa à função de tratamento
        window.addEventListener('languageChanged', this._handleLanguageChange.bind(this));
        
        // 1. Constrói a estrutura visual do componente no DOM
        // 2. Rastreamento de Renderização Inicial
        console.log(`%c[RENDER] %c${this.id}: Iniciando construção do Shadow DOM...`, "color: #6f42c1; font-weight: bold;", "color: #333;");
        this.render(); 
        ...
        queueMicrotask(() => {
            ...
            // Log de Sincronização de Tradução
            console.log(`%c[SYNC] %c${this.id}: Aplicando tradução segura via microtask...`, "color: #007bff; font-weight: bold;", "color: #333;");
            this.translate(); 
        });
    }
    ...
    _handleLanguageChange = (e) => {
        ...
        console.log(`%c[EVENT-DISPATCHED] %c${this.id}: Mudança de idioma detectada!`, "color: #28a745; font-weight: bold;");
    ...
    // REFATORADA PARA MELHORAR A MANUTENÇÃO, CÓDIGO NOVO ACIMA
    // translate() {
