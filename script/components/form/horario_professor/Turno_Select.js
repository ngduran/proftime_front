...
class Turno_Select extends Base_Field {

    static i18n = {
        pt: {
            lbl_turno    : "Turno", 
            ph_turno     : "Selecione um turno",
            tp_lbl_turno : "Turno que leciona na instituição",
            erro         : "Por favor, selecione um turno"       
        },

        es: {
            lbl_turno    : "Cambio",
            ph_turno     : "Seleccione un turno",
            tp_lbl_turno : "Turno que imparte docencia en la institución",
            erro         : "Por favor seleccione un turno."
        }
    };
    ...
    renderControl(p) {   
        ...
        return `<div class="campo">
                    ...
                    <select id="${p.id}" name="${p.name}" class="field-select"
                        ...
                        <option value="Manhã" data-translate="${p.data_translate_op1}"></option>
                        <option value="Tarde" data-translate="${p.data_translate_op2}"></option>
                        <option value="Noite" data-translate="${p.data_translate_op3}"></option>
    ...
    async validarSelect() {
        ...
        if (!valor || valor.trim() === "") {
            const mensagem = Turno_Select.i18n[official_language].erro;
